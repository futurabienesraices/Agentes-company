...
export default function ProspectingCenter() {
  ...
  const results = useMemo(() => memory.filter((item) => (sourceFilter === "Todas" || item.source === sourceFilter) && item.score >= Number(minimumScore)), [memory, minimumScore, sourceFilter]);
  const contactPlan = (item: Memory) => item.legalBasis === "No usar" ? "No contactar. Mantener solo como referencia bloqueada." : item.legalBasis === "Pendiente de revisión" ? "Validar la base de uso antes de preparar contacto." : item.legalBasis === "Consentimiento" ? "Preparar mensaje por el canal autorizado y registrar la respuesta." : "Preparar contacto humano por el canal público o autorizado, con evidencia y siguiente acción.";
  ...
  return <section className="prospecting" aria-label="Centro de prospectos">
    ...
    <div className="sourceGrid">{SOURCES.map((source) => <article key={source.id}><div><small>{source.state}</small><h2>{source.label}</h2><p>{source.description}</p></div>{source.action ? <a href={searchLinks[source.action]} target="_blank" rel="noreferrer">Abrir fuente ↗</a> : <button type="button" disabled>{source.state === "Listo" ? "Disponible" : "Conectar después"}</button>}</article>)}</div>
    ...
    <section className="workspace">
      ...
      <section className="timeline"><header><div><p>RESULTADOS</p><h2>Leads y prospectos encontrados</h2></div><button type="button" onClick={() => void load()} disabled={loading}>{loading ? "Cargando…" : "Actualizar"}</button></header><div className="resultFilters"><select value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value as Source | "Todas")}><option>Todas</option>{SOURCES.map((source) => <option key={source.id}>{source.id}</option>)}</select><select value={minimumScore} onChange={(event) => setMinimumScore(event.target.value)}><option value="0">Todos los puntajes</option><option value="50">50+ puntos</option><option value="70">70+ puntos</option><option value="85">85+ puntos</option></select><span>{results.length} resultados</span></div>{results.length ? <div className="events">{results.map((item) => <button className="prospect" key={item.id} type="button" onClick={() => setSelected(item)}><div className="score"><b>{item.score}</b><span>{item.confidence}% conf.</span></div><div><small>{item.source} · {item.event}</small><h3>{item.prospect}</h3><strong>{item.title}</strong><p>{item.summary}</p></div><span className={`basis ${item.legalBasis === "No usar" ? "stop" : ""}`}>{item.legalBasis}</span></button>)}</div> : <div className="empty">No hay resultados con estos filtros. Registra un hallazgo validado para que aparezca aquí.</div>}</section>
    </section>
    {selected ? <aside className="prospectDetail" aria-label="Ficha de prospecto"><button className="close" type="button" onClick={() => setSelected(null)}>×</button><small>{selected.source} · {selected.event}</small><h2>{selected.prospect}</h2><div className="detailScore"><b>{selected.score}</b><span>puntos · {selected.confidence}% confianza</span></div><h3>{selected.title}</h3><p>{selected.summary}</p><div className={`basis ${selected.legalBasis === "No usar" ? "stop" : ""}`}>{selected.legalBasis}</div><section><strong>Cómo hacerle llegar la información</strong><p>{contactPlan(selected)}</p><em>{selected.recommendedAction}</em></section>{selected.url ? <a href={selected.url} target="_blank" rel="noreferrer">Abrir fuente y evidencia ↗</a> : null}</aside> : null}
    {message ? <p className="message">{message}</p> : null}
    <style jsx>{`
      .prospecting{max-width:1280px;margin:0 auto;padding:6px 0 28px;color:#111827}.hero p,.capture header p,.timeline header p{margin:0 0 7px;color:#6b7280;font-size:.67rem;font-weight:900;letter-spacing:.14em}.hero h1{margin:0;font-size:clamp(2rem,5vw,3.7rem);letter-spacing:-.06em}.hero>div>span{display:block;max-width:670px;margin-top:10px;color:#64748b;font-size:.87rem;line-height:1.5}.search{display:grid;gap:7px;margin-top:22px}.search>span,.fields label>span{color:#64748b;font-size:.69rem;font-weight:850}.search>div{display:flex;align-items:center;gap:10px;padding:0 14px;border:1px solid #dbe3ee;border-radius:16px;background:#fff}.search input{width:100%;height:48px;border:0;outline:0;background:transparent;font:inherit}.search b{color:#2563eb}.sourceGrid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-top:14px}.sourceGrid article{display:flex;min-height:176px;flex-direction:column;justify-content:space-between;padding:15px;border:1px solid #e2e8f0;border-radius:18px;background:#fff}.sourceGrid small{color:#7c3aed;font-size:.62rem;font-weight:850;text-transform:uppercase}.sourceGrid h2{margin:8px 0 6px;font-size:1rem;letter-spacing:-.03em}.sourceGrid p{margin:0;color:#64748b;font-size:.72rem;line-height:1.5}.sourceGrid a,.sourceGrid button{align-self:flex-start;margin-top:13px;border:0;background:none;color:#2563eb;font-size:.72rem;font-weight:850;text-decoration:none}.sourceGrid button:disabled{color:#94a3b8}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}.stats article{padding:15px;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0}.stats strong,.stats span{display:block}.stats strong{font-size:1.65rem;letter-spacing:-.05em}.stats span{margin-top:3px;color:#64748b;font-size:.7rem}.workspace{display:grid;grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr);gap:14px}.capture,.timeline{padding:20px;border:1px solid #e2e8f0;border-radius:22px;background:#fff}.capture h2,.timeline h2{margin:0;font-size:1.35rem;letter-spacing:-.04em}.capture header>span{display:block;margin-top:8px;color:#64748b;font-size:.7rem;line-height:1.45}.fields{display:grid;grid-template-columns:1fr 1fr;gap:11px;margin:18px 0}.fields label{display:grid;gap:6px}.fields .wide{grid-column:1/-1}.fields input,.fields select,.fields textarea,.resultFilters select,.queue select{width:100%;box-sizing:border-box;padding:10px;border:1px solid #dbe3ee;border-radius:11px;background:#fff;font:inherit;font-size:.78rem}.fields textarea{min-height:70px;resize:vertical}.capture>button,.timeline header button{height:40px;padding:0 14px;border:0;border-radius:11px;background:#2563eb;color:#fff;font-weight:850;cursor:pointer}.capture>button:disabled,.timeline header button:disabled{opacity:.55}.timeline header{display:flex;justify-content:space-between;gap:12px;align-items:center}.timeline header button{height:34px;background:#eef3ff;color:#2563eb;font-size:.69rem}.resultFilters{display:flex;gap:8px;align-items:center;margin-top:16px}.resultFilters select{width:auto}.resultFilters span{margin-left:auto;color:#64748b;font-size:.7rem;font-weight:800}.events{display:grid;gap:9px;margin-top:17px}.prospect{display:grid;grid-template-columns:48px minmax(0,1fr) auto;gap:11px;width:100%;padding:12px 0;border:0;border-top:1px solid #edf1f5;background:transparent;color:inherit;text-align:left;cursor:pointer}.prospect:hover h3{color:#2563eb}.score{display:grid;align-content:start;gap:3px}.score b{font-size:1.2rem;color:#2563eb}.score span{color:#94a3b8;font-size:.58rem}.events small{color:#7c3aed;font-size:.62rem;font-weight:850}.events h3,.events strong{display:block}.events h3{margin:5px 0 3px;font-size:.84rem}.events strong{font-size:.73rem}.events p{margin:5px 0;color:#64748b;font-size:.69rem;line-height:1.45}.basis{align-self:start;padding:5px 7px;border-radius:999px;background:#ecfdf5;color:#15803d;font-size:.57rem;font-weight:850}.basis.stop{background:#fef2f2;color:#dc2626}.prospectDetail{position:fixed;z-index:45;inset:0 0 0 auto;width:min(480px,100%);padding:28px;overflow:auto;background:#fff;box-shadow:-18px 0 48px rgba(15,23,42,.16)}.prospectDetail .close{position:absolute;right:14px;top:12px;width:34px;height:34px;border:0;border-radius:50%;background:#f1f5f9;font-size:1.4rem}.prospectDetail>small{color:#7c3aed;font-size:.7rem;font-weight:850}.prospectDetail h2{margin:8px 0;font-size:1.7rem;letter-spacing:-.05em}.detailScore{display:flex;align-items:baseline;gap:8px;padding:13px;border-radius:14px;background:#eef6ff}.detailScore b{font-size:1.5rem;color:#2563eb}.detailScore span{color:#64748b;font-size:.75rem}.prospectDetail h3{font-size:1rem}.prospectDetail>p,.prospectDetail section p{color:#64748b;font-size:.85rem;line-height:1.55}.prospectDetail section{margin-top:20px;padding:14px;border-radius:14px;background:#f8fafc}.prospectDetail section strong,.prospectDetail section em{display:block}.prospectDetail section em{margin-top:9px;color:#334155;font-size:.75rem;font-style:normal}.prospectDetail a{display:block;margin-top:18px;color:#2563eb;font-weight:800;text-decoration:none}.empty{margin-top:18px;padding:22px;border:1px dashed #cbd5e1;border-radius:14px;color:#64748b;font-size:.76rem;line-height:1.5;text-align:center}.message{position:sticky;bottom:16px;z-index:2;margin:16px auto 0;width:max-content;max-width:90%;padding:10px 13px;border-radius:11px;background:#111827;color:#fff;font-size:.73rem;box-shadow:0 12px 30px rgba(15,23,42,.18)}@media(max-width:1020px){.sourceGrid{grid-template-columns:repeat(3,1fr)}.workspace{grid-template-columns:1fr}}@media(max-width:620px){.sourceGrid{grid-template-columns:1fr 1fr}.sourceGrid article{min-height:156px}.stats{grid-template-columns:1fr 1fr}.fields{grid-template-columns:1fr}.fields .wide{grid-column:auto}.prospect{grid-template-columns:42px minmax(0,1fr)}.basis{grid-column:2;justify-self:start}.resultFilters{flex-wrap:wrap}.resultFilters span{margin-left:0}.prospectDetail{padding:20px}}
    `}</style>
  </section>;
}
