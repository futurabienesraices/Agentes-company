...
export default async function ControlPage() {
  ...
  const moduleStates = ["Activa", "Preparada", "Parcial", "Próxima"].map((state) => ({
    label: state,
    value: modules.filter((module) => module.state === state).length,
    detail: `${modules.filter((module) => module.state === state).length} módulos`,
  }));

  const integrationStates = [
    { label: "Conectadas", value: integrations.filter((item) => item.active && !item.paused).length, detail: "Disponibles para operar" },
    { label: "Pausadas", value: integrations.filter((item) => item.paused).length, detail: "Configuradas, sin pruebas activas" },
    { label: "Pendientes", value: integrations.filter((item) => !item.active).length, detail: "Aún sin conexión" },
  ];
  ...
  return (
    <main className="kernelPage">
      ...
      <section className="kernelWorkspace">
        ...
        <section className="kernelCharts">
          <VisualDataView eyebrow="ESTADO DEL KERNEL" title="Madurez de módulos" description="Distribución de capacidades por estado de implementación." data={moduleStates} />
          <VisualDataView eyebrow="CONECTIVIDAD" title="Estado de integraciones" description="Conectadas, pausadas y pendientes en el sistema." data={integrationStates} />
        </section>
